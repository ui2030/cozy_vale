...
# 크림 벽토. 옛 값 (0.880,0.844,0.774)은 정오 직광면이 (255,255,255)로 통째로 날아갔고
# 흐린 비에도 (255,250,235)였다(실측 audit2_0809/beach_spawn_h12 · shots/wall/A_* 재확인).
# 지금 이 색이 칠하는 것: 풍차 탑 · 회관 시계탑 몸통 · 해변 오두막 · 해변 표지판.
# (마을 집·회관 벽은 bb6a8ce에서 GLB 모델로 넘어가 이 상수를 더 이상 안 쓴다 — 그쪽 밝기
#  레버는 HOUSE_GAIN·HOUSE_SAT_CAP이고, 현행 실측으로는 포화하지 않는다.)
#
# **되돌린 시도 두 건** — albedo 하나로는 못 푼다:
#  · 채도를 규칙대로(0.16→0.09) 깎기 → 명도 0.88에서는 B채널까지 255로 포화, 순백 덩어리.
#  · 명도만 0.68까지 내리기 → 직광은 풀리지만 그늘면이 (243,219,211) 파스텔 크림에서
#    (185,156,138) 갈색으로 떨어져 근접 컷이 마을 파스텔 정체성을 잃는다.
# 원인은 값이 아니라 **툰 light()의 lit:그늘 간격이 파스텔 대역보다 넓다**는 것이었다.
# 그래서 이번엔 명도를 내리는 동시에 간격을 좁힌다 = 머티리얼별 shadow_level(ToonChar.WALL_SHADOW).
#
# 이 값이 놓인 자리는 폭이 매우 좁다(양쪽이 다 핀이다):
#  · 위 = 정오 클리핑 상한 0.75 (최대 채널). 넘으면 직광면이 다시 255로 포화한다.
#  · 아래 = 겨울 실루엣 핀 C_WALL.g ≥ C_SNOW.g + 0.15 = 0.725. 밑으로 가면 설원에 묻힌다.
# R−G가 0.025 안에 들어와야 해서 채도가 0.12→0.070으로 내려갔다 — 명도가 0.75 아래라
# 이제 B가 포화하지 않으므로 위 첫 번째 실패는 재발하지 않는다.
...
# 옛 주석은 "벽토가 정오에 255로 포화하므로 창은 포화한 흰 벽을 상대로 대비를 잡는다"였다.
# 그 전제는 이제 없다(벽 직광면 (248,241,234)) — 다만 대비는 여전히 100레벨 이상이라 값은
# 그대로 둔다. 그리고 지금 _window를 부르는 곳은 _house의 박스 폴백뿐인데, GLB 모델이 그
# 앞에서 return하므로 **이 창은 화면에 나오지 않는다**(모델이 제 창을 그린다).
...
# 내릴 때 같은 비율(길 0.915 · 가장자리 0.963)로 따라 내린다 — 안 그러면 길이 설원보다
# 밝아져 "눈밭에 난 흰 띠"가 된다(test_core가 서열을 핀한다).
const C_ROAD_W  := Color(0.516, 0.526, 0.542)
const C_ROAD_WE := Color(0.539, 0.554, 0.577)  # 겨울 길 가장자리 — 설원으로 스밈
...
#
# 0.66/0.68/0.71도 부족했다(실측 season_audit/winter_clear_life_h12 = 이 값의 컷):
# 지평선 설원이 (228,234,245)라 벽(255)과 차이가 21뿐이고 **색상까지 같은 계열**이다.
# 근경(211,214,222)은 ground.gdshader 패턴이 깎아준 값이라 착시였고, 건물이 지면과 만나는
# 지평선 띠에서는 패턴이 거리 페이드(fade_dist 22~48)로 사라져 albedo 그대로 뜬다 —
# 즉 실루엣을 판정하는 자리가 화면에서 제일 밝은 설원이다. 여기에 외곽선까지 없어졌다(2bde841).
# 0.555/0.575/0.605 → 지평선 설원 (185,193,209): 벽과 차이 62로 윤곽이 읽히고
# 근경 요철도 유지된다. 툰 light()는 면 방향으로 명암을 거의 안 가르므로(수평 지면과 수직
# 벽이 같은 shade=1.0) **화면 대비를 만들 수 있는 것은 albedo 차뿐**이다.
