...
const KEYS := [
	...
	# 1.375라 초지가 밤에 청록으로 뒤집혔다(실측 21시: G 0.196 < B 0.220). 지금 amb B/G는 1.18,
	# 태양까지 합친 실효 광색의 B/G는 1.19로 여전히 1.25 아래다 = 초록 유지.
	#
	# 그 위에 두 가지를 더 잡는다("보라 필터 낀 낮" 교정, 실측 audit2_0809/*_h21):
	#  · **감광** — 지면 휘도가 80으로 하늘(100)과 거의 같아 밤 같지 않았다. amb 0.40→0.34,
	#    sun 0.18→0.12로 실효 광량을 0.66배 = 지면이 하늘 아래로 내려앉는다.
	#  · **탈채도** — 옛 amb는 R<G(0.36<0.41)라 초지의 초록을 오히려 밀어 올려 대낮 잔디로 읽혔다.
	#    R을 G 위로(0.32>0.30) 올려 보랏빛 하늘과 같은 계열로 두면 잔디가 회록으로 앉는다.
	#    (G를 깎아 탈채도하는 길은 B/G가 1.25를 넘어 청록 뒤집힘 — 반드시 R을 올리는 쪽으로.)
	{"h": 0.0,  "sun_rot": Vector3(-58, -125, 0), "sun_col": Color(0.56, 0.63, 0.76), "sun_e": 0.12, "amb_col": Color(0.32, 0.30, 0.355), "amb_e": 0.34},
	{"h": 5.0,  "sun_rot": Vector3(-58, -125, 0), "sun_col": Color(0.56, 0.63, 0.76), "sun_e": 0.12, "amb_col": Color(0.32, 0.30, 0.355), "amb_e": 0.34},
	...
	# 18시 "단색 오렌지 뭉갬"을 여기서 노출을 내려 풀어 보려 했으나 실패했다(실측: 0.95/0.52 →
	# 0.86/0.46으로 프레임 전체가 4% 어두워질 뿐 지면·벽 분리는 8.0→7.6단, 즉 무변화). 툰 light()가
	# 면 방향을 거의 안 가르므로 조명 세기는 **모든 면에 같은 배율**로 걸린다 = 분리를 못 만든다.
	# 실제 뭉갬은 물이었고(강 165 ≈ 흙둑 154) water.gdshader에서 고쳤다. 승인된 노을 값은 그대로 둔다.
	...
	{"h": 21.0, "sun_rot": Vector3(-58, -125, 0), "sun_col": Color(0.56, 0.63, 0.76), "sun_e": 0.12, "amb_col": Color(0.32, 0.30, 0.355), "amb_e": 0.34},
	{"h": 24.0, "sun_rot": Vector3(-58, -125, 0), "sun_col": Color(0.56, 0.63, 0.76), "sun_e": 0.12, "amb_col": Color(0.32, 0.30, 0.355), "amb_e": 0.34},
]
