...
const NO_DECOR_CIRCLES := [
	...
	[Vector2(5.0, 5.8), 2.0],    # 판매 상자 (프롬프트) — 밭 북서 모서리(world.tscn ShippingBin)
...
const FARM := Rect2(3.2, 6.2, 9.6, 5.6)           # 밭 타일 x[4,12] z[7,11] + 0.8 (farm_system.REGION)
...
const PROPS := [
	...
	# 벤치 — 광장 3(분수 향) + 정자 양옆 2. yaw = atan2(dx,dz) 규약(바라보는 방향).
	# 옛 강변 벤치(12.40, 8.60)은 밭이 그 레인으로 오면서 강변 난간과 자리가 겹쳤다 —
	# 난간은 강을 따라가야 하니 점 소품인 벤치를 광장 남동 림으로 보냈다.
	["bench", 4.30, 0.91, -102], ["bench", -3.21, -3.57, 42],
	["bench", 4.30, 3.90, -132], ["bench", -29.20, 14.00, 90], ["bench", -22.80, 14.00, -90],
...
const FENCES = [
	# 밭 북쪽 울타리(광장에서 내려오는 쪽 경계). 남쪽엔 안 둔다 — 밭(z≤11)과 집 북벽(z12.5)
	# 사이 1.5 통로를 막고, 작물 컷 카메라가 그 자리에 서서 프레임을 통째로 먹는다(실측).
	# 서쪽은 판매상자, 동쪽은 동마을길이라 가운데 한 칸만 선다.
	[Vector2(7.0, 6.0), Vector2(9.6, 6.0)],
	...
	# 동 다리 남측 강변 난간 = 밭 동쪽 울타리 겸용. 옛 x11.4는 이제 밭 안이라 동쪽으로 물렸다
	# (북단은 다리 keepout 4.5, 남단은 강이 (16,12)에서 꺾어 RIVER_KEEP — 둘 사이가 이 구간이다).
	[Vector2(12.9, 7.9), Vector2(12.9, 11.9)],
