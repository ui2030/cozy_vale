...
const FARM := Rect2(3.2, 7.2, 9.6, 5.6)           # 밭 타일 x[4,12] z[8,12] + 0.8 (farm_system.REGION)
...
const PROPS := [
	...
	["lamp", 7.40, 13.40, 0],   # 플레이어 집 북동 마당 (밭 남쪽 — 옛 6.6,12.6은 밭 금지존 안)
	["lamp", 2.60, -14.60, 0],  # 회관 앞
	# 벤치 — 광장 2(분수 향) + 강변 1(다리 향) + 정자 양옆 2
	["bench", 4.30, 0.91, -102], ["bench", -3.21, -3.57, 42],
	["bench", 12.90, 9.00, 109], ["bench", -29.20, 14.00, 90], ["bench", -22.80, 14.00, -90],
...
const FENCES = [
	[Vector2(4.0, 7.0), Vector2(9.2, 7.0)],       # 밭 북쪽 — 판매상자 쪽 한 칸은 금지존에 걸려 출입구가 된다
	...
	# 남쪽 절반은 강이 (16,12)에서 남서로 꺾여 RIVER_KEEP에 걸리므로 한 칸만 선다.
	[Vector2(12.9, 9.6), Vector2(12.9, 12.0)],
