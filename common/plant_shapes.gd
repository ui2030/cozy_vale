...
const SHAPES := {
	...
	# 야생 씨앗(늦가을) — 곧게 선 좁은 잎 넷 사이로 작은 알이 뭉쳐 달린 씨앗 꼬투리. 종 색은
	# 그 종의 열매 색을 그대로 쓰므로 **형태는 넷이 공유한다**: "이건 씨앗이다"가 먼저 읽히고
	# 어느 종인지는 색이 말한다. 잎을 거의 세우고(25°) 알을 잎 위쪽(0.62)에 얹어야 채집 어휘의
	# 잎다발 다섯 종과 안 뭉친다.
	"seed_pod":       ["tuft", 0.40, 0.100, 4, 25, 0.052, 0.62, 5],
